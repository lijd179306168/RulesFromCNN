...
#loadrules, use filters as follows
s1= [22, 96, 110, 191, 231, 239, 242, 267, 317, 384, 398, 435]
s2= [22, 96, 110, 191, 231, 239, 242, 267, 317, 384, 398, 435]

def getpos(s1,s2):
    pos=[]
    for i in range(len(s1)):
        tmppos=s2.index(s1[i])
        pos.append(tmppos)
    return pos  
pos=getpos(s1,s2)
# load_rules
...
for i in range(len(rules)):
    ...
    for j in range(len(tmprule)):
        pop1[i][pos[j]]=tmprule[j]  #fill rules
...
def evaluate(g,rule):
    ...
    for eachexample in traindata:
        ...
        if triangle.getRuleMF(x,rule)>0.5:
            ...
            if y!=rule.classLabel:
                miscount=miscount+1 
    conditionsCount=triangle.getRuleConditons(rule)
    fit=correctcount/eachClassAmout[rule.classLabel]-0.2*miscount/eachClassAmout[rule.classLabel]-0.1*conditionsCount/ruleLength   
